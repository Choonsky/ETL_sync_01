CREATE OR REPLACE PACKAGE IN_DATA.DATA_PACKAGE AS
    PROCEDURE addSomeData(rowsCount NUMBER);
END DATA_PACKAGE;

CREATE OR REPLACE PACKAGE BODY IN_DATA.DATA_PACKAGE AS

    PROCEDURE addSomeData(rowsCount NUMBER) IS
        v_data       in_data.data_main%ROWTYPE;
        rowsCountInt NUMBER := 100000;
    BEGIN
        if rowsCount is not null then rowsCountInt := rowsCount;
        end if;
